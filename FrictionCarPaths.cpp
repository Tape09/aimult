...
FrictionCarPaths::FrictionCarPaths(FVector pos0_, FVector vel0_, FVector pos1_, FVector vel1_, float v_max_, float phi_max_, float L_car_, float a_max_, float k_friction_) {
	...
	addTransforms(std::bind(&FrictionCarPaths::get_path_LSL, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LSR, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LGRGL, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LGRL, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LRGL, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LRGLR, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LGRLGR, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LGR90SL, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LSR90GL, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LGR90SR, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LSL90GR, this, std::placeholders::_1), goal_state);
	addTransforms(std::bind(&FrictionCarPaths::get_path_LGR90SL90GR, this, std::placeholders::_1), goal_state);
...
float FrictionCarPaths::time_taken(int idx) {
	return turn_radius * all_paths[idx].calc_dist() / v_max;
}

float FrictionCarPaths::time_taken() {
	return turn_radius * all_paths[0].calc_dist() / v_max;
}


State FrictionCarPaths::state_at(RSPath rsp, float time) const {
...
State FrictionCarPaths::state_at(State istate, RSComponent rsc, float dist) const {
	State s;
...
State FrictionCarPaths::drive_R(State istate, RSComponent rsc, float dist) const {
	State s;
...
State FrictionCarPaths::drive_S(State istate, RSComponent rsc, float dist) const {
	State s;
	float theta0 = vecAngle(istate.vel);
	dist = rsc.gear * dist;
...
State FrictionCarPaths::state_at(int idx, float time) const {
	return state_at(all_paths[idx], time);
}

int FrictionCarPaths::n_paths() const {
	return all_paths.size();
}


float FrictionCarPaths::theta(float t) const {
	return v_max*t / turn_radius;
}

State FrictionCarPaths::step(float delta_time) {
	t_now += delta_time;
...
//1: 8.1
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LSL(const RSState & goal) {
	float t = 0;
	float u = 0;
	float v = 0;
...
//2: 8.2
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LSR(const RSState & goal) {
...
//3: 8.3
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LGRGL(const RSState & goal) {
	RSPath out_path;
...
//4: 8.4
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LGRL(const RSState & goal) {
	RSPath out_path;
...
//5: 8.4
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LRGL(const RSState & goal) {
	RSPath out_path;
...
//6: 8.7
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LRGLR(const RSState & goal) {
	RSPath out_path;
...
//7: 8.8
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LGRLGR(const RSState & goal) {
	RSPath out_path;
...
//8: 8.9
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LGR90SL(const RSState & goal) {
	RSPath out_path;
...
//9: 8.9
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LSR90GL(const RSState & goal) {
	RSPath out_path;
...
//10: 8.10
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LGR90SR(const RSState & goal) {
	RSPath out_path;
...
//11: 8.10
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LSL90GR(const RSState & goal) {
	RSPath out_path;
...
//12: 8.11
FrictionCarPaths::RSPath FrictionCarPaths::get_path_LGR90SL90GR(const RSState & goal) {
	RSPath out_path;
...
void FrictionCarPaths::addTransforms(pathFcn fptr, const RSState & goal) {
	RSPath rsp;
	RSState rs;
...
float FrictionCarPaths::path_time(int idx) const {
	return all_paths[idx].time;
}

float FrictionCarPaths::path_time() const {
