...
DifferentialDrivePaths::DifferentialDrivePaths(FVector pos0_, FVector vel0_, FVector pos1_, FVector vel1_, float v_max_, float w_max_) {
	...
	for (int i = 0; i < radii_fractions.size(); ++i) {
		...
		RSState goal_state((pos1 - pos0) / r, wrapAngle(vecAngle(vel1) - vecAngle(vel0)));
		rotateVector(goal_state.pos, -vecAngle(vel0));

		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LSL, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LSR, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LGRGL, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LGRL, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LRGL, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LRGLR, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LGRLGR, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LGR90SL, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LSR90GL, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LGR90SR, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LSL90GR, this, std::placeholders::_1), goal_state);
		addTransforms(std::bind(&DifferentialDrivePaths::get_path_LGR90SL90GR, this, std::placeholders::_1), goal_state);
	}	
...
float DifferentialDrivePaths::time_taken() {
	return all_paths[0].time;
}


State DifferentialDrivePaths::state_at(RSPath rsp, float time) const {
...
State DifferentialDrivePaths::state_at(State istate, RSComponent rsc, float time) const {
	State s;
...
State DifferentialDrivePaths::drive_R(State istate, RSComponent rsc, float time) const {
	State s;
	float theta0 = vecAngle(istate.vel);

	float theta = rsc.gear * rsc.v * time / rsc.r;
	float LL = 2 * sin(theta / 2) * rsc.r;
...
State DifferentialDrivePaths::drive_L(State istate, RSComponent rsc, float time) const {
	State s;
	float theta0 = vecAngle(istate.vel);
...
State DifferentialDrivePaths::drive_S(State istate, RSComponent rsc, float time) const {
	State s;
	float theta0 = vecAngle(istate.vel);
...
DifferentialDrivePaths::RSPath DifferentialDrivePaths::get_path_LGRGL(const RSState & goal) {
	...
	out_path.components.push_back(RSComponent(L, 1, t, w_max, v_now, r));
	out_path.components.push_back(RSComponent(R, -1, -u, w_max, v_now, r));
	out_path.components.push_back(RSComponent(L, 1, v, w_max, v_now, r));
	out_path.is_valid = true;
	out_path.calc_dist();
	out_path.calc_time();
...
DifferentialDrivePaths::RSPath DifferentialDrivePaths::get_path_LRGLR(const RSState & goal) {
	...
	out_path.components.push_back(RSComponent(L, 1, t, w_max, v_now, r));
	out_path.components.push_back(RSComponent(R, 1, u, w_max, v_now, r));
	out_path.components.push_back(RSComponent(L, -1, -u, w_max, v_now, r));
	out_path.components.push_back(RSComponent(R, -1, -v, w_max, v_now, r));
	out_path.is_valid = true;
	out_path.calc_dist();
	out_path.calc_time();
...
void DifferentialDrivePaths::addTransforms(pathFcn fptr, const RSState & goal) {
	RSPath rsp;
	RSState rs;
...
float DifferentialDrivePaths::path_time(int idx) const {
	return all_paths[idx].time;
}
